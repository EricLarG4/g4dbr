---
title: "DatAnirban 0.32"
author: "Eric Largy"
date: "Quarantine, day 34"
output:
  pdf_document:
    df_print: kable
    highlight: tango
...
+ _MS_ can now be filtered by _tunes_ and _replicates_
  - _Tune_ names and _replicate_ numbers filter now available in the _right sidebar_ of the MS plot box
...
+ _meltR_ now manages data that cannot be baseline-subtracted
  - Export raw data button added in _left sidebar_
...
The circular dichroism data is presented as a scatter plot colored by buffer and shaped by oligonucleotide (both changeable). The data can be shown in mdeg or molar ellipticities (default), and filtered by wavelength. Molar ellipticities are calculated automatically from the supplied mdeg data.
...
![Info template](Figures/template_info.PNG)
...
![CD template. Four spectra have been added. Note that one of the x-axis is mismatched](Figures/template_cd.PNG)
...
![NMR template](Figures/template_nmr.PNG)
...
![NMR labels template. Note that both oligonucleotides have completely different labelling](Figures/template_nmr_labels.PNG)

#### MS.

The MS template is identical to the NMR template, albeit that column one is _m/z_. 

![MS template](Figures/template_ms.PNG)
...
msfilter <- function(file = NULL, minmz = 1150, maxmz = 1650, basestart = 1250, baseend = 1450){
  ...
  df <- df %>%
    ...
    write_xlsx(path = 'Figures\\filteredmz_markdown.xlsx')
}
```

```{r msfull, echo=FALSE}
read_xlsx('Figures\\raw_ms_markdown.xlsx') %>%
  filter(mz > 1150) %>%
  filter(mz < 1650) %>%
...
unfiltered <- msfilter(file = 'tools\\unfilteredmz.xlsx', 
         minmz = 1150, maxmz = 1650, 
         basestart = 1250, baseend = 1450)

read_xlsx('Figures\\filteredmz_markdown.xlsx') %>%
  ggplot(aes(x = mz, y = int)) +
  geom_line() +
  theme_pander() +
  xlab('m/z') +
...
![MS labels template. Note the difference in labelling between oligonucleotides and buffer. Both charge states have been labelled the same however](Figures/template_ms_labels.PNG)
